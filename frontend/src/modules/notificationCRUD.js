...
const useNotification = () => {
  ...
  const getNotifications = async (username) => {
    try {
      const res = await axios.get(`https://bmp-inv-be.zenbytes.id/notification/${username}`)
  ...
  const markAsRead = async (id) => {
    try {
      await axios.put(`https://bmp-inv-be.zenbytes.id/notification/mark-read/${id}`)
  ...
  const markAllAsRead = async (username) => {
    try {
      await axios.put(`https://bmp-inv-be.zenbytes.id/notification/mark-all-read/${username}`)
